...
@UntilDestroy()
@Component({
  selector: 'cc-create-activity-variant',
  template: `
    <div class='illustration'
         style='background-image: url("https://via.placeholder.com/400x600.png/09f/fff?text=une+sorte+de+fork")'>
    </div>
    <div *ngIf='(this.facade.vm$ | async) as vm' class='form'>
      <form [formGroup]='facade.form' id='createActivityVariantForm' (ngSubmit)='facade.submit()' errorTailor>
  ...
  providers: [CreateActivityVariantFacadeService]
})
export class CreateActivityVariantComponent {

  constructor(public readonly facade: CreateActivityVariantFacadeService, private router: Router, private route: ActivatedRoute) {
